...
def test_init():
    ...
    assert board.board == [['P', 'P', 'P', 'P', 'P'],
                           ...
                           [' ', ' ', ' ', ' ', ' '],
                           ['N', 'N', 'N', 'N', 'N']]
    assert board.directions == {'up': (-1, 0), 'down': (1, 0), 'left': (0, -1), 'right': (0, 1), 'up-right': (-1, 1), 'up-left': (-1, -1), 'down-right': (1, 1), 'down-left': (1, -1)}  # noqa: E501
    assert board.neutron_moved is False
    assert board.current_piece is None
    assert board.move_count == 0
    assert board.first_move is None
...
def test_find_neutron():
    ...
    board.board[2][2] = ' '
    assert board.find_neutron() is None
...
def test_move_piece_up():
    ...
    initial_pos = (2, 2)
    direction = 'up'
    assert board.move_piece(*initial_pos, direction) is True
...
def test_move_piece_invalid():
    ...
    board.board = [['P', 'P', 'P', 'P', 'P'],
                   ...
                   [' ', ' ', 'O', ' ', ' '],
                   ['N', 'N', 'N', 'N', 'N']]
    assert board.move_piece(3, 2, 'down') is False
...
def test_is_valid_move():
    ...
    assert board.is_valid_move(2, 2, 'up') is True
    assert board.is_valid_move(0, 4, 'left') is False
